...
所以用 [`ai/docs/2026-08-04-act-v2-deploy.md`](2026-08-04-act-v2-deploy.md) 第三節逐幀確認過的
時間點造了一支：`test8.mp4` 的 **9.0~12.5s**（10.0s 是「該人整個趴在地上」、13.0s 已站起），
重複 18 次接成 **62.3 秒**。產生腳本與素材都是暫存的，不進版控；作法完整記在本節，可重現：
...
python -m pytest agent ai scripts -q     → 191 passed in 3.94s
...
- **真攝影機**：三輪都是 mp4。冷卻這個機制當初就是為了「worker 一跑好幾天」的
  真攝影機情境而做的，但 RTSP 長時間運行仍然沒有實測過。
- **`FALL_COOLDOWN_SEC` 的預設值 180 秒合不合用**：本輪為了在單支影片內走完冷卻窗
  一律縮短成 5 秒。180 這個數字仍然只是「改動前等於無限長，保守往長抓」的推測值，
  沒有現場資料支持。
- **多台相機各自的冷卻互不干擾**：粒度設計是每台相機 × 每種事件類型，但三輪都只掛一路
  （`SINGLE_SOURCE`），沒有實際跑多路確認 dict 的 key 不會互相蓋掉。
