...
用法：
    python ai/submit_task.py            # 排一張單（已有進行中的就跳過）
...
PIPELINE = os.path.join(AI_DIR, "clearml_train_pipeline.py")


def submit(force: bool = False) -> int:
    ...
    if not force:
        ...
        if active:
            print(f"🛑 已有任務 {active[0].id} 在執行/排隊中，跳過重複點火"
                  f"（要強制再排一張加 --force）")
            return 0
    ...
    task.set_parameters({
        ...
        "ai_dir": AI_DIR,
        "epochs": os.environ.get("TRAIN_EPOCHS", "60"),
        "batch": os.environ.get("TRAIN_BATCH", "4"),
        "device": os.environ.get("TRAIN_DEVICE", "0"),
        "map50_gate": os.environ.get("MAP50_GATE", "0.80"),
    })
    task.set_base_docker(None)
    task.set_packages([])          # 不讓 agent 自行裝套件，沿用本機 ai/.venv
    task.output_uri = True

    print(f"📦 任務建立成功：{task.id}")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="把 RT-DETR 重訓任務排進 ClearML 佇列")
    ap.add_argument("--force", action="store_true",
                    help="即使已有進行中/排隊中的同名任務也照排（預設會跳過）")
    return submit(force=ap.parse_args().force)
