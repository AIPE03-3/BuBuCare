...
| 延伸功能 | ① 通報單自動草稿 ② 事件問答助手 ③ 主動學習樣本篩選（取代寫死的 0.35–0.85 規則）。**值班日報不做。** |
...
| 慢節點與告警路徑的關係（2026-07-20） | **對告警沒必要的 LLM 節點一律排在 publish 之後或移出圖外**。已套用：`al_curator` 移到 publish 之後（省 3.2 秒）、通報單草稿移出 Kafka 路徑 |
