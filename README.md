...
欄位依長照通報的實際格式排列，涵蓋個案基本資料、事發時間、行政區、發生地點與影響程度。事發時間與地點由事件本身帶入，不需要人工回填，這是把偵測結果接進既有行政流程的銜接點。
...
分成已結報事件、誤報紀錄、已排除危險三個分頁。誤報紀錄獨立成一頁是刻意的，它同時是稽核軌跡與重訓素材的來源，護理人員標記誤報的每一筆，都會回到第十二節的資料飛輪。
...
角色分為系統管理者與護理站值班人員兩種，僅管理者可進入本頁。帳號以工號識別，事件的處理人欄位即對應到這裡的帳號，使每一次複判與通報都能追溯到人。
...
<details>
<summary><b>系統架構圖</b>　各層元件與資料流全貌</summary>

<br>

實線為影像資料，虛線為事件資料，點線為模型。由上而下依序是邊緣運算層、
事件匯流與低信心複審、儲存與資料服務、控制與管理平面，以及 MLOps 迴路。

![系統架構圖](docs/assets/architecture.png)

> 圖中兩處與現行程式碼有出入，都是已知且刻意保留的：
> 影像接收與解碼標示 DeepStream，那是具備 NVIDIA GPU 的正式環境下的目標架構，
> 現行實作為 PyAV，取捨見 [4.3](#43-解碼層與平台差異)；
> VLM 標示 Qwen3.5 為選型結論，程式碼尚未切換，見 [7.3.1](#731-選型實測)。
