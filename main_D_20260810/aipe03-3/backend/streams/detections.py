...
class DetectionFrame(BaseModel):
    ...
    source_pts_ms: Optional[int] = None
    rtsp_received_at_ms: Optional[int] = None
    decoder_output_at_ms: Optional[int] = None
    inference_start_at_ms: Optional[int] = None
    inference_end_at_ms: Optional[int] = None
    deepstream_frame_at_ms: Optional[int] = None
    ai_sent_at_ms: Optional[int] = None
    backend_received_at_ms: Optional[int] = None
...
async def push_detections(frame: DetectionFrame) -> dict[str, Any]:
    frame.backend_received_at_ms = time.time_ns() // 1_000_000
    payload = frame.model_dump()
    _latest[frame.device_id] = payload
    # Log one sample per 30 frames per camera. This is frequent enough for
    # diagnosis without flooding normal service logs.
    if frame.seq is not None and frame.seq % 30 == 0:
        ds_to_ai = (
            frame.ai_sent_at_ms - frame.deepstream_frame_at_ms
            if frame.ai_sent_at_ms and frame.deepstream_frame_at_ms else None
        )
        ai_to_backend = (
            frame.backend_received_at_ms - frame.ai_sent_at_ms
            if frame.ai_sent_at_ms else None
        )
        logger.warning(
            "E2E device=%s seq=%s rtsp_to_decoder_ms=%s decoder_to_infer_ms=%s "
            "inference_ms=%s ds_to_ai_ms=%s ai_to_backend_ms=%s source_pts_ms=%s",
            frame.device_id,
            frame.seq,
            (frame.decoder_output_at_ms - frame.rtsp_received_at_ms)
            if frame.decoder_output_at_ms is not None and frame.rtsp_received_at_ms is not None else None,
            (frame.inference_start_at_ms - frame.decoder_output_at_ms)
            if frame.inference_start_at_ms is not None and frame.decoder_output_at_ms is not None else None,
            (frame.inference_end_at_ms - frame.inference_start_at_ms)
            if frame.inference_end_at_ms is not None and frame.inference_start_at_ms is not None else None,
            ds_to_ai,
            ai_to_backend,
            frame.source_pts_ms,
        )
