...
5. **🔴 骨架與影像不同步（canvas 疊圖帶進來的退步）。** 2026-07-31 端到端實測發現
   骨架會**超前**影像：MediaMTX 對慢讀取端主動丟舊幀，推論拿到最新畫面（延遲約
   250ms），而瀏覽器的 WebRTC 路徑有 jitter buffer（500ms 以上），兩條管線延遲不同。
   舊的 `detect_publisher` 把框燒進影像走同一條管線，**構造上就永遠同步**，沒有這個
   問題。2026-07-31 決議先接受並記錄（該機是 CPU 版 Triton、推論僅 ~4fps，正式機上
   落差會小很多）。要修的兩條路與完整實測數據見
   [`TEST-PLAN-E2E.md`](TEST-PLAN-E2E.md) 階段 2。
