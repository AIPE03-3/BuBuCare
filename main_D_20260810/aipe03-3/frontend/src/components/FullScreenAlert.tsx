...
export function FullScreenAlert({ alerts, now, onAcknowledge, onSuppress, onDismiss }: FullScreenAlertProps) {
  ...
  // A notification may arrive while the user is on another browser tab. Never
  // let its event video play in the background, and do not auto-resume when the
  // tab becomes visible again; the user must explicitly select Play again.
  useEffect(() => {
  ...
  const hasLiveStream = Boolean(activeAlert.camera.stream_url && activeAlert.camera.stream_channel);
  const playbackEnabled = playbackId === activeAlert.id;
      ...
      <div className="flex w-full max-w-3xl min-h-0 flex-col overflow-y-auto rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] p-4 shadow-sm sm:p-6">
        ...
        <div className="mt-4 aspect-video max-h-[55vh] w-full overflow-hidden rounded-xl bg-[var(--bg-surface-2)]">
          {!playbackEnabled ? (
            <button
              type="button"
              onClick={() => setPlaybackId(activeAlert.id)}
              className="flex h-full w-full items-center justify-center text-base font-medium text-[var(--text-primary)] hover:bg-[var(--bg-surface-2)]"
            >
              點擊播放此事件（從跌倒前 3 秒開始）
            </button>
          ) : clipLoading ? (
            <div className="flex h-full items-center justify-center">
              <span className="text-[var(--text-muted)]">載入事件影片中</span>
            </div>
          ) : clipUrl ? (
            <video
              key={activeAlert.id}
              ...
              onLoadedMetadata={(event) => {
                const match = videoSrc.match(/#t=([0-9.]+)/);
            ...
            <LiveStream
              whepUrl={activeAlert.camera.stream_url}
              channel={activeAlert.camera.stream_channel}
              overlayDeviceId={null}
              emptyLabel="找不到事發鏡頭即時影像"
            />
          ) : videoError ? (
            <div className="flex h-full items-center justify-center">
        ...
        <p className="mt-3 text-xs text-[var(--text-muted)]">
          顯示實際觸發事件的鏡頭即時影像；也可按右上角叉叉關閉。按「接手處理」即轉為「已接手」；標記「誤報」需二次確認。
        </p>
      </div>
