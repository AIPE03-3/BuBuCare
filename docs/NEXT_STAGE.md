...
| 15 | VLM 判讀與畫面對不上 | 🔴 P0 | 🆕 **2026-08-05 實測發現**，判讀說「沒有看到人」卻仍 `true_alarm`（本檔下方）|
| 16 | ByteTrack 換號會繞過去重 | 🟠 P1 | 🆕 **2026-08-05 實測發現**，2 分鐘發 9 筆（本檔下方）|
...
## 15.【2026-08-05 實測發現】VLM 判讀與畫面對不上，但仍判 `true_alarm`

**怎麼發現的**：2026-08-05 用 `ai/test_demo/test1.mp4` 推進 MediaMTX `phone_a`
（`rtsp://127.0.0.1:8554/phone_a`）、以 `SINGLE_SOURCE` 跑真實串流鏈路，
2 分鐘產生 9 筆事件。後端 `GET /events` 撈回來看 `vlm_summary`：

| 事件 | `ai_verdict` | `vlm_summary` 開頭 |
|---|---|---|
| `..._134416_p8.jpg` | `true_alarm` | 「1. 人數：**沒有看到人**。2. 身體姿態：沒有看到人。」 |
| `..._134344_p7.jpg` | `true_alarm` | 「1. 人數: **沒看到人**。」 |
| `..._134312_p6.jpg` | `true_alarm` | 「1. 人數：**沒有看到人**。」 |
| `..._134311_p5.jpg` | `true_alarm` | 「1. 人數: 1　2. 身體姿態: **站立**」 |

**兩個獨立的問題**：

1. **VLM 看不到人**（前三筆）。快照本身有沒有人要先確認——可能是
   `ai/snapshots/` 的裁切/取幀時機不對，也可能是 VLM 對這個畫面就是判不出來。
   ⚠ 先看圖再下結論，不要憑 `vlm_summary` 推測。
2. **judge 不看 VLM 的結論**。第四筆 VLM 明說「站立」，仍然出 `true_alarm`。
   這呼應 `agent/docs/04-open-questions.md` 早就記過的「篩選能力接近零、
   judge 的安全偏向過強」——但那份是 5 張圖的結論，這次是端到端實測再次確認。

**為什麼重要**：agent 存在的目的就是幫值班人員濾掉誤報。目前它每一筆都往上報，
**安全，但完全沒有減輕負擔**，等於白跑一趟 VLM（每筆約 30 秒）。

**動手前要先做的**：把那幾張快照調出來人眼看過，確定是「圖沒有人」還是「VLM 看不出來」——
這兩者的修法完全不同（前者修取幀，後者修 prompt 或換模型）。

---

## 16.【2026-08-05 實測發現】ByteTrack 換號會繞過去重，短時間重複發報

**現象**：同一段 2 分鐘的串流，發出 9 筆事件，track id 分別是
`1 → 14 → 26 → 41 → 52 → 53 → 61 → 81 → 84`。每個新號都被當成「新的一位」各發一筆。
同一份 log 裡去重機制擋掉了 24 次，**擋得住號碼相近的，擋不住位置漂移超過門檻的**。

**現行去重的條件**（`ai/inference_test.py:189-196`）：
`_FALL_DEDUP_SEC=30` 秒內、且中心距離 < `_FALL_DEDUP_DIST=0.6` 個身位，才算同一人。

**但這次的放大效應有一半是測試手法造成的**，要誠實說明：
測試是 `-stream_loop -1` 迴圈播放同一支影片，人物每輪都從頭走一次，
位置漂移遠超過 0.6 個身位。**真實攝影機不會 loop**，所以實際場域不會這麼誇張。

**仍然要修的理由**：ByteTrack 換號本身是真的（`ai/inference_test.py:186-188`
的註解記載 test6 三個人出現過 6 個 track_id），只是這次被測試手法放大了。
真實場域裡「人在地上翻動、被遮擋後換號」一樣會漏過 0.6 身位的門檻。

**修的方向（尚未決定）**：
- 調 `FALL_DEDUP_SEC` / `FALL_DEDUP_DIST`（最省事，但門檻放寬會開始擋掉真的兩個人）
- 或改用「相機級冷卻」蓋住換號窗口——但那正是第 5 項當初刻意**不**套在有追蹤器路徑上的原因
  （會漏接冷卻期間別人的跌倒），要一起評估
- ⚠ 量測前先確認測試素材不是 loop 播放，否則數字會失真
