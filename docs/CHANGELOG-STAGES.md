...
**項目編號刻意沿用原本的**（第 1、2、3、5、6、7、8、9、10、14 項）—— 程式碼註解與其他文件
有多處引用「第 N 項」「缺陷三」，改號會把它們全部打斷。

> **讀這份的心法**：這裡記的是「當時做了什麼、為什麼那樣做、踩過哪些坑」。
> 有價值的多半不是結論本身，而是**「為什麼用當時的測法測不出來」**那幾段。
> 想知道系統現在長什麼樣、哪裡還是壞的，看 [`ARCHITECTURE.md`](ARCHITECTURE.md)。

**第 9 項有一個缺陷至今未修**（缺陷四 體角與俯視佈署不符，缺陷三 `normal_h_reference`
已於 2026-08-03 隨第 5 項一起修掉），雖然整項標【已完成】。未修的部分在
[`NEXT_STAGE.md`](NEXT_STAGE.md) 有指標。
...
## 5.【已完成】跌倒事件不再只發一次：一次性閂鎖改成冷卻計時器

分支 `fix/fall-cooldown-and-href-reset`，與第 9 項缺陷三（`normal_h_reference` 換來源
不重設）同一批一起修，兩個都是接真攝影機、worker 跑好幾天才會現形的問題。

### 原本壞在哪

`ai/inference_test.py` 的 `vlm_triggered` 是 per-worker 的一次性旗標，同一路相機
**整個行程生命週期只發一次跌倒事件**，片段錄製掛在同一個閂鎖下所以也只錄一支。
影片檔時代沒事（播完就收工），接真攝影機後 worker 一跑好幾天，等於第一次觸發
（很可能是誤報）之後那台相機就變成裝飾品。實測時也踩到：AI 啟動 9 秒就被
方向問題誤觸發（見第 9 項），把唯一額度用掉，後面真的躺下完全不會再發事件，
一度以為是事件管線壞了——不解開這個閂鎖，現場調校與驗收測試幾乎沒辦法做，
每測一次就要重啟一次 AI。

### 動手時才確認的關鍵事實：死鎖只鎖得到一半的路徑

`vlm_triggered` 這把鎖**只鎖得到「ByteTrack 起不來」那條退路**。多人分案進 main
之後，有追蹤器時走的是 per-track 閂鎖（`st["fired"]` + 站起來 `FALL_RECOVERY_FRAMES`
個處理幀解鎖），那條早就能重複發報了。所以修法只動退路那一段：

- **冷卻只套退路路徑**（`person_tracker is None`）。相機級冷卻若套到有追蹤器那條上，
  會擋掉冷卻期間**別人**的跌倒——那正是這項當初被擱置的理由（`NEXT_STAGE.md` 第 5 項
  舊版寫的封鎖原因），多人分案進來後這個前提才消失，兩件事因此排在同一批做。
- **重新武裝＝冷卻到期 `and` 畫面連續 `FALL_RECOVERY_FRAMES` 個處理幀判非跌倒**
  （`_fallback_rearm()`，有單元測試）。純看時間的話，人倒著沒人處理會週期性洗版，
  也會變成「追蹤器有起來就不重複提醒、追蹤器掛了反而每 3 分鐘提醒一次」的反直覺行為。
- `FALL_COOLDOWN_SEC` 未設＝**180 秒**（改動前等於無限長，保守方向偏長）。
  粒度是每台相機（worker 本來就一路一個）× 每種事件類型。
- **斷線重連不重設冷卻**（維持原本保留 `vlm_triggered` 的用意：網路抖動不該重複發報）。
- 片段錄製一行都沒改：它掛在 `is_recording_post`（寫檔後歸零）＋「有沒有進到發事件
  迴圈」，閂鎖一放行第二支自然錄得到。
...
`84c9920` 當時的實測涵蓋了「發得出第二筆 + 第二支片段」，但**沒有涵蓋「冷卻期間不會
週期性洗版」**——那要專門造一支「人倒下後就一直躺著」的素材才驗得到，而
`ai/test_demo/` 裡沒有。加上當時線上還是 AcT v1、二審還是 `vlm_worker`，
所以在 AcT v2 + `agent/` 的現況上重跑了一輪，完整紀錄在
[`ai/docs/2026-08-04-cooldown-e2e.md`](../ai/docs/2026-08-04-cooldown-e2e.md)。

三項全過，**沒有改任何一行程式**：

| 驗收 | 做法 | 結果 |
|---|---|---|
| ① 發得出第二筆 | `MULTI_PERSON_TRACK=0 FALL_COOLDOWN_SEC=5 FALL_RECOVERY_FRAMES=10` 跑 `test7.mp4` | 2 筆，中間有「解除本路相機的 fall 事件閂鎖」那行 |
| ② 第二支片段也錄到 | 同上 | 兩支檔名/大小/md5/幀數全不同，S3 presigned URL 下載回 HTTP 200 且大小與本機一致 |
| ③ 冷卻期間不洗版 | 把 `test8.mp4` 的 9.0~12.5s（人趴在地上）重複 18 次接成 62 秒，冷卻設 5 秒 | 12 個冷卻窗到期，**只發 1 筆、解鎖 0 次** |

對照組（有追蹤器 + 冷卻 180 秒跑 `test7.mp4`）照樣發 4 筆，證實冷卻確實只套退路路徑。
端到端 Kafka +7 / `processed-reports` +7 / 後端事件 +7，1:1 無重複無掉件。

**同日補測：多路相機的冷卻互不干擾**（Run D／Run E，`STRESS_CAM_COUNT=3` 掛三路
`Room_301/302/303`，一樣走 `MULTI_PERSON_TRACK=0` 的退路路徑）：

- **Run D**（冷卻 180 秒）：三路在 17:22:55／17:22:58／17:23:01 各發一筆，前後只差 6 秒。
  狀態若跨相機共用，後兩路會被壓到 17:25:55，整支測試只會有 1 筆。實際 3 筆。
- **Run E**（冷卻 5 秒）：`Room_302`／`Room_303` 解了閂鎖，`Room_301` **一次都沒解**——
  三路的冷卻到期時間幾乎一樣，差別出在 `_fallback_rearm()` 的另一半（301 的 `test1.mp4`
  只有 187 幀，跑完前沒湊滿 10 個非跌倒幀）。狀態共用的話不可能分歧。
- 程式面的根因：`fall_cooldown_until`／`cam_armed`／`cam_recovery_frames` 是
  `camera_worker()` 的**函式區域變數**，而一路相機開一條執行緒 —— 隔離來自變數作用域，
  不是來自 dict 的 key。
- 端到端：兩輪共 6 筆，Kafka `nursing-home-alerts` +2 / `processed-reports` +6、
  後端事件 39→45，無重複無掉件，agent lag 0。

**仍未驗（刻意不驗，使用者指示一律以拍好的影片測試）**：真攝影機長時間運行、
`FALL_COOLDOWN_SEC=180` 這個預設值合不合用、`event_type` 第二種值域。

---

## 14.【已完成】VLM 二審模型從 `llava:latest` 切到 `qwen2.5vl:7b`

分支 `feat/vlm-qwen25vl`。團隊先前已拍板 `qwen2.5vl:7b`（`agent/docs/04-open-questions.md`），
但本機一直沒裝、也從未在同一台機器上對同一批圖做過頭對頭驗證——`.env` 沒設
`VLM_MODEL_NAME`，`ai/vlm_client.py` 預設值仍是 `llava:latest`，等於正式二審長期跑著
團隊自己否決掉的模型。

### 先補頭對頭 A/B，通過才切

用真實 TAPO 跌倒片段（`ai/snapshots/tapo_raw/milkteafall.mp4` 擷取 6 張：3 張跌倒瞬間、
3 張事發前正常走動），兩模型跑同一批圖、同一個 prompt（逐字取自
`ai/uncertainty_router.py` 的非巡檢分支），完整記錄在
[`ai/docs/2026-08-03-vlm-ab.md`](../ai/docs/2026-08-03-vlm-ab.md)。結論：

| 面向 | 結果 |
|---|---|
| 跌倒判讀正確性 | `qwen2.5vl` 3/3 精準對應畫面內容；`llava` 3/3 方向大致對，但 2/3 有明顯敘述失準或場景幻覺（如把走廊講成病房） |
| JSON 契約合規（主動學習用）| `qwen2.5vl` 6/6 格式正確；`llava` 2/6 有格式問題（多餘跳脫符號、陣列取代單一物件）|
| 拒答 | 兩者 0/6，打平，本輪未重現舊觀察到的 `llava` 隨機拒答 |
| **簡體字外洩** | `llava` 2/6；`qwen2.5vl` **0/6**——直接推翻先前「`qwen2.5vl` 才會回簡體」的既定印象，該印象可能來自不同版本/機器/prompt，本次重測才發現方向反了 |
| 延遲（熱機後平均）| `llava` 6.93s、`qwen2.5vl` 8.85s，慢約 28%，但都遠在 `AGENT_VLM_TIMEOUT_SECONDS`（120s）容忍範圍內 |

`qwen2.5vl:7b` 正確率不輸（甚至更準）、JSON 合規率更高、簡體字外洩率更低，延遲雖略慢
但可接受——**閘門通過**。

### 切換的 8 個檔案（一起改，讓「預設值＝拍板值」，`.env` 漏了也不會崩）

`.env`（新增 `VLM_MODEL_NAME=qwen2.5vl:7b`、`AGENT_VLM_MODEL=qwen2.5vl:7b`）、
`agent/config.py`（`AGENT_LLM` 預設也順手改成 `ollama:qwen2.5:7b`，原本預設指向
本機沒裝的 `qwen3`）、`ai/vlm_client.py`、`agent/llm.py`、`agent/tests/conftest.py`、
`agent/tests/test_llm.py`、`agent/tests/test_main.py`、`.env.example`。

⚠️ **`ai/vlm_client.py` 讀的是 `os.environ`，不是 `.env`**（`docs/MAC_SETUP_WBS.md:134`），
`vlm_worker.py` 啟動指令要 export `VLM_MODEL_NAME` 或寫在指令前，不能只改 `.env` 檔就以為生效。
...
都不會發現的問題**。前三個已修，第四個已診斷定位、依決議留待與模型調校一起處理。
...
| 3 | `normal_h_reference` 換來源不重設 | ✅ 2026-08-03 已修（分支 `fix/fall-cooldown-and-href-reset`，見下）|
...
### 缺陷三：`normal_h_reference` 換來源不重設（**2026-08-03 已修**）
...
**修法（分支 `fix/fall-cooldown-and-href-reset`，與第 5 項同一批）**：畫面尺寸變化
（換攝影機／改構圖／以不同解析度重連）時把 `normal_h_reference` 設回 `None`、同時把
每個 track 的 `h_ref` / `h_frames` 也清掉（逐人參考身高同樣是用舊尺寸量的）。
