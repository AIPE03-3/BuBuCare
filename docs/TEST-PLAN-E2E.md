...
> 註（2026-08-10）：`al_curator` 已移除，現在是 2 次 Ollama 呼叫。上面的秒數是移除前
> 實測的，維持原值 —— 改成「2 次」會讓這組數字變成在描述一場沒發生過的測試。
> 見 [`CHANGELOG-STAGES.md`](CHANGELOG-STAGES.md) 第 15 項。
...
4. **🟡 主動學習收樣把「模型本來就判對的」也收進去。** 7/7 全收、全部
   ...
   **誤報**」，`agent_verdict` 欄位卻是 `true_alarm`。照這樣收下去，重訓資料集會被
   ...
   > 註（2026-08-10）：這第 4 項不再修了 —— 整個 `al_curator` 節點移除，見
   > [`CHANGELOG-STAGES.md`](CHANGELOG-STAGES.md) 第 15 項。它診斷出來的東西反而是
   > 移除的直接證據：收進來的樣本連「哪一類盲點」都說反了。
...
| 快照 presigned URL 有**顯示在前端**                       | ❌ **沒接**（見下）|

### 2026-07-31 實測紀錄

驗的是修完快照上傳（`85ab27f`）後那筆事件 `1961fee4`：

```
GET /events/{id}/media  →  clip_url / snapshot_url 兩張都簽得出來
curl 下載               →  clip 200 video/mp4 235,005B ／ snapshot 200 image/jpeg 317,750B
```

**唯讀金鑰確實可用**——presigned URL 是後端用 `ACCESS_KEY_ID` 那組簽的，下載走的是簽章，
不帶任何本機憑證，等於同時驗到了「寫（`S3_RW_*`）」與「讀（`S3_*`）」兩組。

**新舊事件對照**（證明修的是對的）：

| 事件 | `snapshot_path` | `snapshot_url` |
|---|---|---|
| `1961fee4`（修完後）| `s3://aipe03-3/snapshots/...jpg` | ✅ 簽得出來 |
| `e71df079`（修之前）| `/Users/.../ai/snapshots/...jpg` | ❌ `null` |

`generate_presigned_url()` 只認 `s3://`，非 s3:// 直接回 `None` 而**不是報錯**——
這就是為什麼這個缺陷 179 筆事件都沒被發現。

**片段編碼**（PyAV 解出）：`h264 / High / yuv420p / 640×360 / 20fps` — Safari 播得動。

**片段長度**：本機 8 支片段有 7 支剛好 `200 幀 = 10.00s`（前 5 + 後 5）。
唯一的例外是最後那支 110 幀 —— 影片檔播完、後段還沒錄滿就被 flush 出去
（`inference_test.py:642` 那條分支，刻意的：寧可短也不要留一個指向空檔的 `clip_path`）。

**觸發點對位**：抽 `clip_..._172114.mp4` 的第 100 幀（＝片段第 5.0 秒＝觸發瞬間），
畫面正是人倒地那一刻（相機 OSD `14:56:08`，雙腿still在空中）。前後文都在。

**無框**：片段與快照全程都是原始畫面，沒有任何 bbox／骨架／文字疊加。✅

**快照**：1920×1080 原尺寸，一人躺在走廊地上、另一人蹲著查看 —— 與 VLM 那句
「一名男性躺在走廊地上，另一名男性正蹲下查看其情況」完全對得上。

### 這階段查出來的三件事

1. **🔴 `snapshot_url` 前端從頭到尾沒接。** `useEventClipUrl` 只取 `media.clip_url`，
   `snapshot_url` 拿了就丟。`ActionRequiredCard` 與 `HazardDetail` 該放縮圖的位置
   是寫死的灰色占位框 `CAMERA_LABEL.SNAPSHOT_PLACEHOLDER`（「事件快照（影像片段）」）。
   → 後端這條路已經全通，缺的只是前端接線。

2. **🟡 faststart 沒生效。** 片段的 atom 順序是 `ftyp / free / mdat / moov`，
   `moov` 在檔尾。`_write_frames_h264_pyav()` 設了 `movflags=+faststart`，但那是
   **第二順位**的路；macOS 上 OpenCV 的 `avc1`（VideoToolbox）第一順位就成功了，
   PyAV 那段根本沒跑到，所以那個選項對本機產出的片段一律無效。
   實務影響小：S3 支援 Range（實測 `206`），瀏覽器會自己去抓檔尾的 moov，
   只是多一次 round-trip，不是播不出來。

3. **🟡 播不出來時會靜靜換成 demo 影片。** `EventDetail.tsx:49`
   `const videoSrc = clipUrl ?? DEMO_VIDEO_SRC` —— presigned 失敗時畫面上照樣有影片在播，
   看不出是壞的。demo 期間很好用，上線前該讓它顯性失敗。
