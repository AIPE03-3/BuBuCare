...
| [`NEXT_STAGE.md`](../NEXT_STAGE.md) | 待辦與各項狀態、踩坑記錄 |
| **本檔** | **架構全貌與設計理由** |
...
同樣的思路：`al_curator`（挑重訓樣本）刻意排在 `publish` **之後** —— 它是一次 LLM 呼叫
（實測 3.2 秒），而產出沒有任何人急著讀。排前面等於讓跌倒通知白等 3.2 秒。
...
詳見 [`NEXT_STAGE.md`](../NEXT_STAGE.md) 第 9 節。**不要以為跌倒偵測在所有場景都可靠。**
