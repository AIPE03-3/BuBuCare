...
if echo "$CMD" | grep -q "backend.main:app"; then
	...
	# 改寫如下:
    docker exec -d -w "$BACKEND_IN_CONTAINER" "$PY_CONTAINER" \
    sh -c 'PYTHONDONTWRITEBYTECODE=1 exec uv run uvicorn backend.main:app --host 0.0.0.0 --port 8000 --reload --reload-dir backend > /proc/1/fd/1 2> /proc/1/fd/2'
fi
# PYTHONDONTWRITEBYTECODE=1 是 Python 的環境設定，意思是：不要在磁碟上寫入 Python 編譯快取檔(__pycache__/ 這些)
# 單引號 的 exec uv run ...（建議保留）：這是 Linux Shell 的內建指令。 
