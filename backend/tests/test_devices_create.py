...
def test_admin_creates_device_with_stream_url(client, db_session):
    # admin 新增一台指定房號的攝影機 → 201，回應是完整的裝置結構、DB 也存進去了
    res = client.post("/devices", headers=_admin_headers(client), json={
        "device_id": 301,
        "device_name": "寢室-301",
        "stream_url": "rtsp://127.0.0.1:8554/cam301",
        "location_id": 1,
    })
    ...
    assert device.stream_url == "rtsp://127.0.0.1:8554/cam301"
...
def test_created_device_appears_in_list(client, auth_headers):
    # AI 端實際走的路徑：新增完之後 GET /devices 要看得到，且 stream_url 一字不差
    url = "rtsp://user:pass@192.168.1.20:554/stream2"
    created = client.post("/devices", headers=_admin_headers(client), json={
        "device_id": 302, "device_name": "寢室-302", "stream_url": url,
    })
...
def test_too_long_stream_url_returns_422(client):
    # DB 欄位是 String(255)，超長要在 pydantic 就擋下，不要掉到資料庫變 500
    res = client.post("/devices", headers=_admin_headers(client),
                      json={"device_name": "超長網址", "stream_url": "rtsp://" + "a" * 300})
    assert res.status_code == 422
